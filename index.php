...
//Creo la sesión que guarde el filtro para ordenar
if(!isset($_SESSION['filtro'])){
    //Y por defecto lo ordeno por el nombre, alfabeticamente
    $_SESSION['filtro']="first_name";
}
...
if ($_SERVER['REQUEST_METHOD'] == "GET" ){
    ...
    if ( isset($_GET['nav-detalles']) && isset($_GET['id']) ) {
        switch ( $_GET['nav-detalles']) {
            ...
            //Añado el botón de imprimir
            case "Imprimir": crudDetallesImprimir($_GET['id']); break;
        }
    }

    //Añado los mismos botones para la pagina de modificar
    ...
    if ( isset($_GET['orden'])){
        switch ($_GET['orden']) {
            ...
            //Añado el caso de que se quiera ordenar, y hago que coja el botón que ha pulsado
            case "Filtrar"  : $_SESSION['filtro']=$_GET['valor'];
        }
    }
} 
...
if ( ob_get_length() == 0){
    ...
    //Añado la variable para el filtro a la hora de ordenar
    $filtro=$_SESSION['filtro'];
    //Y se lo paso a la variable tvalores
    $tvalores = $db->getClientes($posini,FPAG,$filtro);
